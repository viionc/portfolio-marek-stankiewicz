...
function Project({project}: {project: ProjectProps}) {
    ...
    return (
        <motion.div
            ...
            <article className="group bg-white bg-opacity-60 rounded-lg shadow-md overflow-hidden border border-black/25 sm:pr-8 relative max-w-[42rem]  hover:bg-opacity-50 transition">
                <div className="py-4 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] text-gray-900 flex flex-col h-full">
                    ...
                    <p className="mt-2 leading-relaxed text-gray-700 ms-auto">{project.description}</p>
                    <div className="flex flex-wrap gap-2 justify-center py-2">
                        {project.technologies.map((technology, index) => (
                            <span key={index} className="bg-zinc-500 text-white rounded-md px-2 py-1">
                                {technology}
                            </span>
                        ))}
                    </div>
                    <div className="flex justify-center items-center gap-4 sm:mt-auto pb-2 pt-4">
                        <a
                            href={project.source}
                            target="_blank"
